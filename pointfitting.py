...
#             for k in range(profis[j - 1].shape[1]):
#                 functs[i][j] = functs[i][j] + profis[j - 1][0][k] * numpy.exp(-1 * (xscatt[i] - profis[j - 1][1][k])**2 / (2 * profis[j - 1][2][k]**2)) / (profis[j - 1][2][k] * (2 * numpy.pi)**.5)
#     fig = mpl.figure()
